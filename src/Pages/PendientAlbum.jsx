...
export default function PendientAlbum() {
  const [albumPend, serAlbumPend] = useState(artists.filter(artist => artist.listen === false));
  ...
  const addAlbum = () => {
    ...
    serAlbumPend([...albumPend, newAlbum]);
    setName('');
    setAlbum('');
    setYear('');
  };

  const albumPendient = albumPend.map((a) => (
  ...
  return (
    <>
      <div className='container mx-auto p-10'>
        <h1 className='text-red-900 font-bold'>Albums pendientes</h1>
        <ul className='bg-red-500 grid grid-cols-4 gap-2 border-slate-800 border-2'>{albumPendient}</ul>
      </div>
      <div>
